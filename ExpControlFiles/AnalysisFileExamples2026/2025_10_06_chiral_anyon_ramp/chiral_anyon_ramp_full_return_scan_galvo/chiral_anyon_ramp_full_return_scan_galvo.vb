...
doublon_splitting = 0
stop_time = 0
new_small_galvo_dim = 0
new_big_galvo_dim = 0
free_var = 0
'=====Variables=====
'===================
small_galvo_dim = new_small_galvo_dim
big_galvo_dim = new_big_galvo_dim
'-------------------------------------------------------------------------------------------------------------------------------------------------- Constants
...
Dim loadline_DMD_volt_vert As Double = 3.3 '3.3
Dim loadline_DMD_volt_horz As Double = 3.2 '3.1
Dim DMD_hw_delay As Double = -0.16

'Lattice ramp constants
Dim lattice_freeze_dur As Double = 1

' B field ramp constants
Dim grad_onoff_dur As Double = 30
Dim loading_grad_onoff_dur As Double = 60
...
Dim Berlin_wall_expansion_volt As Double = 3.05 '2.7 '3.05

'For doublon splitting
Dim splitting_lattice_dur As Double = 30
Dim splitting_turnon_dur As Double = 30
Dim splitting_ramp_dur As Double = 300
Dim lattice2_doublon_depth As Double = 15
Dim splitting_quic_start As Double = 6.52 '5.5
Dim splitting_quic_end As Double = 2.1 '3
...
'Dim loading_grad_turnon_start_time As Double = line_load_end_time 
...
Dim mod_hold_dur As Double = 0
Dim mod_phase_hold_dur As Double = 0
...
If (is_return > 0) Then
	If (phase_ramp_dur > 0) Then
		mod_hold_dur = grad_ramp_dur
		mod_phase_hold_dur = hold_dur + grad_ramp_dur
	Else
		mod_hold_dur = 2*grad_ramp_dur + hold_dur
	End If
Else
	If (phase_ramp_dur > 0) Then
		mod_hold_dur = grad_ramp_dur
		mod_phase_hold_dur = hold_dur
	Else
		mod_hold_dur = grad_ramp_dur + hold_dur
	End If
End If
...
Dim lattice_raise_end_time As Double = lattice_raise_start_time + lattice_ramp_dur
If (is_return > 0) Then
...
'() RETURN: raise old wall pattern
Dim r_loading_walls_turnon_start_time As Double = walls_turnoff_end_time + 5
Dim r_loading_walls_turnon_end_time As Double = r_loading_walls_turnon_start_time
If (is_full_return > 0) Then
	r_loading_walls_turnon_end_time = r_loading_walls_turnon_start_time + walls_onoff_dur
End If

'() RETURN: turn on compensation gradient
Dim r_loading_grad_comp_turnon_start_time As Double = r_loading_walls_turnon_end_time
Dim r_loading_grad_comp_turnon_end_time As Double = r_loading_grad_comp_turnon_start_time
If (is_full_return > 0) Then
	r_loading_grad_comp_turnon_end_time = r_loading_grad_comp_turnon_start_time + grad_onoff_dur
End If

'() RETURN: lower gradient
Dim r_loading_grad_lower_start_time As Double = r_loading_grad_comp_turnon_end_time
Dim r_loading_grad_lower_end_time As Double = r_loading_grad_lower_start_time
If (is_full_return > 0) Then
	r_loading_grad_lower_end_time = r_loading_grad_lower_start_time + grad_onoff_dur
End If

'() RETURN: lower lattice
Dim r_loading_lattice_lower_start_time As Double = r_loading_grad_lower_end_time
Dim r_loading_lattice_lower_end_time As Double = r_loading_lattice_lower_start_time
If (is_full_return > 0) Then
	r_loading_lattice_lower_end_time = r_loading_lattice_lower_start_time + loading_lattice_ramp_dur
End If

'() RETURN: raise gradient
Dim r_loading_grad_raise_start_time As Double = r_loading_lattice_lower_end_time
Dim r_loading_grad_raise_end_time As Double = r_loading_grad_raise_start_time
If (is_full_return > 0) Then
	r_loading_grad_raise_end_time = r_loading_grad_raise_start_time + loading_grad_ramp_dur
End If

'() RETURN: raise lattice
Dim r_loading_lattice_raise_start_time As Double = r_loading_grad_raise_end_time
Dim r_loading_lattice_raise_end_time As Double = r_loading_lattice_raise_start_time
If (is_full_return > 0) Then
	r_loading_lattice_raise_end_time = r_loading_lattice_raise_start_time + lattice_freeze_dur
End If

'() RETURN: lower walls
Dim r_loading_walls_turnoff_start_time As Double = r_loading_lattice_raise_end_time
Dim r_loading_walls_turnoff_end_time As Double = r_loading_walls_turnoff_start_time
If (is_full_return > 0) Then
	r_loading_walls_turnoff_end_time = r_loading_walls_turnoff_start_time + walls_onoff_dur
End If

'() RETURN: turn off compensation gradient
Dim r_loading_grad_comp_turnoff_start_time As Double = r_loading_walls_turnoff_end_time
Dim r_loading_grad_comp_turnoff_end_time As Double = r_loading_grad_comp_turnoff_start_time
If (is_full_return > 0) Then
	r_loading_grad_comp_turnoff_end_time = r_loading_grad_comp_turnoff_start_time + grad_onoff_dur
End If

' --------- Full counting methods

'() Doublon splitting
Dim quic_turnon_splitting_start_time As Double = r_loading_grad_comp_turnoff_end_time
...
Dim full_counting_hold_dur As Double = 1.2 '2.3
Dim full_counting_rampup_dur As Double = 1
Dim full_counting_hold_after_dur As Double = 10 '10
...
Else
	...
	analogdata.AddStep(lattice1_max_volt, line_load_end_time, pinning_ready_time, lattice2D765_power) '2D1
End If
...
' ---- Modulation
If (phase_ramp_dur > 0) Then
	' ramp up and ramp phase
	If (is_return > 0) Then
		' Return ramp modulation
		'analogdata.AddRampLogSine3PhaseReturn(lattice2_low_depth, latticemod_amp_1, latticemod_amp_2, latticemod_amp_3, mod_freq_Hz_1, mod_freq_Hz_2, mod_freq_Hz_3, phase_pi_init, phase_pi_final, phase_ramp_dur, phase_ramp_start_time, lattice_mod_ramp_dur, mod_hold_dur + mod_phase_hold_dur, lattice2_voltage_offset, lattice2_calib_volt, lattice2_calib_depth, lattice2D765_power2)
		analogdata.AddRampLogSine3PhaseReturn(lattice2_low_depth, latticemod_amp_1, latticemod_amp_2, latticemod_amp_3, mod_freq_Hz_1, mod_freq_Hz_2, mod_freq_Hz_3,
											  phase_pi_init, phase_pi_final, phase_ramp_dur, phase_ramp_start_time, lattice_mod_ramp_dur, mod_hold_dur + mod_phase_hold_dur,
											  lattice2_voltage_offset, lattice2_calib_volt, lattice2_calib_depth, lattice2D765_power2)
	Else
		'analogdata.AddRampLogSine3NoReturn(lattice2_low_depth, latticemod_amp_1, latticemod_amp_2, latticemod_amp_3, mod_freq_Hz_1, mod_freq_Hz_2, mod_freq_Hz_3, phase_pi_init, mod_start_time, lattice_mod_ramp_dur, mod_hold_dur, lattice2_voltage_offset, lattice2_calib_volt, lattice2_calib_depth, lattice2D765_power2)
		'analogdata.AddRampLogSine3PhaseNoReturn(lattice2_low_depth, latticemod_amp_1, latticemod_amp_2, latticemod_amp_3, mod_freq_Hz_1, mod_freq_Hz_2, mod_freq_Hz_3, phase_pi_init, phase_pi_final, phase_ramp_dur, phase_ramp_start_time, lattice_mod_ramp_dur, mod_phase_hold_dur, lattice2_voltage_offset, lattice2_calib_volt, lattice2_calib_depth, lattice2D765_power2)
		analogdata.AddRampLogSine3PhaseNoReturn(lattice2_low_depth, latticemod_amp_1, latticemod_amp_2, latticemod_amp_3, mod_freq_Hz_1, mod_freq_Hz_2, mod_freq_Hz_3,
												phase_pi_init, phase_pi_final, phase_ramp_dur, mod_start_time, lattice_mod_ramp_dur, mod_hold_dur, 
												lattice2_voltage_offset, lattice2_calib_volt, lattice2_calib_depth, lattice2D765_power2)
		analogdata.AddLogSine3Phase(lattice2_low_depth, latticemod_amp_1, latticemod_amp_2, latticemod_amp_3, mod_freq_Hz_1, mod_freq_Hz_2, mod_freq_Hz_3, 
									phase_pi_final, hold_start_time, hold_end_time, hold_start_time-mod_start_time, lattice2_voltage_offset, lattice2_calib_volt, lattice2_calib_depth, lattice2D765_power2)
	End If
Else
	If (is_return > 0) Then
		' ramp up and return
		analogdata.AddRampLogSine3Return(lattice2_low_depth, latticemod_amp_1, latticemod_amp_2, latticemod_amp_3, mod_freq_Hz_1, mod_freq_Hz_2, mod_freq_Hz_3, phase_pi_init, mod_start_time, lattice_mod_ramp_dur, mod_hold_dur, lattice2_voltage_offset, lattice2_calib_volt, lattice2_calib_depth, lattice2D765_power2)
	Else
		' ramp up
		'analogdata.AddRampLogSine3NoReturn(lattice2_low_depth, latticemod_amp_1, latticemod_amp_2, latticemod_amp_3, mod_freq_Hz_1, mod_freq_Hz_2, mod_freq_Hz_3, phase_pi_init, mod_start_time, lattice_mod_ramp_dur, mod_hold_dur, lattice2_voltage_offset, lattice2_calib_volt, lattice2_calib_depth, lattice2D765_power2)
...
analogdata.AddStep(lattice2_max_volt, lattice_raise_end_time, r_loading_lattice_lower_start_time, lattice2D765_power2)

'State prep return
If (is_full_return > 0) Then
	' Lower 2D2
	analogdata.AddSmoothRamp(lattice2_max_volt, loading_lattice2_low_volt, r_loading_lattice_lower_start_time, r_loading_lattice_lower_end_time, lattice2D765_power2)
	analogdata.AddStep(loading_lattice2_low_volt, r_loading_lattice_lower_end_time, r_loading_lattice_raise_start_time, lattice2D765_power2)

	' Raise 2D2
	analogdata.AddSmoothRamp(loading_lattice2_low_volt, lattice2_max_volt, r_loading_lattice_raise_start_time, r_loading_lattice_raise_end_time, lattice2D765_power2)
	analogdata.AddStep(lattice2_max_volt, r_loading_lattice_raise_end_time, lower_lattice_splitting_start_time, lattice2D765_power2)
Else
	analogdata.AddStep(lattice2_max_volt, r_loading_lattice_lower_start_time, lower_lattice_splitting_start_time, lattice2D765_power2)
End If
...
End If
analogdata2.AddStep(quic_grad_end, grad_raise_end_time, r_loading_grad_lower_start_time, ps5_ao)

' State prep return
If (is_full_return > 0) Then
	' PS 5 (quic)
	analogdata2.AddSmoothRamp(quic_grad_end, loading_grad_final*ps5_scaler, r_loading_grad_lower_start_time, r_loading_grad_lower_end_time, ps5_ao)
	analogdata2.AddStep(loading_grad_final*ps5_scaler, r_loading_grad_lower_end_time, r_loading_grad_raise_start_time, ps5_ao)
	analogdata2.AddSmoothRamp(loading_grad_final*ps5_scaler, loading_grad_init*ps5_scaler, r_loading_grad_raise_start_time, r_loading_grad_raise_end_time, ps5_ao)
	analogdata2.AddStep(loading_grad_init*ps5_scaler, r_loading_grad_raise_end_time, quic_turnon_splitting_start_time, ps5_ao)
	quic_grad_end = loading_grad_init*ps5_scaler
	' PS 6 (quic mirror)
	analogdata.AddSmoothRamp(0, loading_grad_comp, r_loading_grad_comp_turnon_start_time, r_loading_grad_comp_turnon_end_time, ps6_ao)
	analogdata.AddStep(loading_grad_comp, r_loading_grad_comp_turnon_end_time, r_loading_grad_comp_turnoff_start_time, ps6_ao)
	analogdata.AddSmoothRamp(loading_grad_comp, 0, r_loading_grad_comp_turnoff_start_time, r_loading_grad_comp_turnoff_end_time, ps6_ao)
Else
	analogdata2.AddStep(quic_grad_end, r_loading_grad_lower_start_time, quic_turnon_splitting_start_time, ps5_ao)
End If
...
If (doublon_splitting > 0) Then
	...
	analogdata2.AddRamp(splitting_quic_start*ps5_scaler, splitting_quic_end*ps5_scaler, quic_ramp_splitting_start_time, quic_ramp_splitting_end_time, ps5_ao)
...
digitaldata2.AddPulse(hor_DMD_trigger, DMD_hw_delay + r_loading_walls_turnon_start_time - 0.5, DMD_hw_delay + r_loading_walls_turnon_start_time) 'Walls loading return (hor DMD)
...
' walls2 state prep return (hor DMD)
If (is_full_return > 0) Then
	If (loading_walls_volt > 0) Then
		digitaldata2.AddPulse(hor_DMD_ttl, r_loading_walls_turnon_start_time - PID_response_dur/2, r_loading_walls_turnoff_end_time + PID_response_dur/2)
		analogdata2.AddSmoothRamp(line_DMD_start_volt, loading_walls_volt, r_loading_walls_turnon_start_time, r_loading_walls_turnon_end_time, hor_DMD_power)
		analogdata2.AddStep(loading_walls_volt, r_loading_walls_turnon_end_time, r_loading_walls_turnoff_start_time, hor_DMD_power)
		analogdata2.AddSmoothRamp(loading_walls_volt, line_DMD_start_volt, r_loading_walls_turnoff_start_time, r_loading_walls_turnoff_end_time, hor_DMD_power)
	End If
End If
