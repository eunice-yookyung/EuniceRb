...
namespace DLPGUI
{
    public partial class DMD_Control_GUI : Form
    {
        ...
        */
        ...
        private void button_log_Click(object sender, EventArgs e)
        {
            ...
            if (System.IO.Directory.Exists(folderBrowserDialog1.SelectedPath + "\\hor_DMD_log") == true)
                MessageBox.Show("'DMD_log' already existed, logging aborted. Delete folder first.");
            else
            {
                System.IO.Directory.CreateDirectory(folderBrowserDialog1.SelectedPath + "\\hor_DMD_log");
                System.IO.Directory.CreateDirectory(folderBrowserDialog1.SelectedPath + "\\hor_DMD_log\\current");

                System.IO.File.Copy(System.IO.Directory.GetParent(".").Parent.FullName + "\\Form1.cs", folderBrowserDialog1.SelectedPath + "\\hor_DMD_log\\Form1.cs");
                System.IO.File.Copy("console_log.txt", folderBrowserDialog1.SelectedPath + "\\hor_DMD_log\\console_log.txt");
                //System.IO.File.Copy("arb_sequence.txt", folderBrowserDialog1.SelectedPath + "\\DMD_log\\arb_sequence.txt");
                System.IO.File.Copy("arb_sequence.txt", folderBrowserDialog1.SelectedPath + "\\hor_DMD_log\\" + DateTime.Now.ToString("yyyyMMdd") + "arb_sequence.txt");

                // copy the current patterns
                System.IO.DirectoryInfo di = new System.IO.DirectoryInfo("patterns\\current");
                foreach (System.IO.FileInfo file in di.GetFiles()) System.IO.File.Copy(file.FullName, folderBrowserDialog1.SelectedPath + "\\hor_DMD_log\\current\\" + file.Name, true);

                using (var bmp = new Bitmap(this.Width, this.Height))
                {
                    this.DrawToBitmap(bmp, new Rectangle(0, 0, bmp.Width, bmp.Height));
                    //bmp.Save(@"c:\temp\screenshot.png");
                    bmp.Save(folderBrowserDialog1.SelectedPath + "\\hor_DMD_log\\DMD_param.png");
                }

                //System.IO.File.Copy("patterns\\grating\\000.raw", "patterns\\measureProfile\\" + index.ToString("D3") + ".raw", true);
                using (System.IO.StreamWriter file = new System.IO.StreamWriter(folderBrowserDialog1.SelectedPath + "\\hor_DMD_log\\DMD_params.txt", false))
                {
     ...
     * */
        ...
        {
            string log_pathname = "Z:\\Data\\DMD_tracking";
            string log_filename = "dmd_tracking_current_hor.txt";
        ...
        private void OnTrackingFileChanged(object source, FileSystemEventArgs e, string log_file)
        {
            try
            {
                ...
                if (((timeNow.Ticks - convertedTime.Ticks) / TimeSpan.TicksPerMillisecond) > 5000)
                {
                    ...
                    //dlp.tracking_shift_x.Add(Convert.ToDouble(tracking_shifts[0].Trim()));
                    //dlp.tracking_shift_y.Add(Convert.ToDouble(tracking_shifts[1].Trim()));

                    //double tracking_shift_x = 0;
                    //double tracking_shift_y = 0;

                    ////if there are at least 3 values stored in the list, then average them.
                    ////otherwise, just be directly proportional
                    //int i=0;
                    //if (dlp.tracking_shift_x.Count()>2)
                    //{   
                    //    for (i = 0; i<3 ; i++)
                    //    {
                    //        tracking_shift_x = tracking_shift_x + dlp.tracking_shift_x[dlp.tracking_shift_x.Count() - i - 1];
                    //        tracking_shift_y = tracking_shift_y + dlp.tracking_shift_y[dlp.tracking_shift_y.Count() - i - 1];
                    //    }
                    //    i = 3;
                    //}
                    //else{
                    //    for (i = 0; i < dlp.tracking_shift_x.Count(); i++)
                    //    {
                    //        tracking_shift_x = tracking_shift_x + dlp.tracking_shift_x[dlp.tracking_shift_x.Count() - i - 1];
                    //        tracking_shift_y = tracking_shift_y + dlp.tracking_shift_y[dlp.tracking_shift_y.Count() - i - 1];
                    //    }
                    ...
                    //}
                    //tracking_shift_x = tracking_shift_x / (i);
                    //tracking_shift_y = tracking_shift_y / (i); 

                   

                    Console.WriteLine("New shifts for DMD tracking: (" + tracking_shift_x.ToString() + ", " + tracking_shift_y.ToString() + ") - " + File.GetLastWriteTime(log_file).ToString());
                    ...
                    dlp.piezo_hist_x[2] =  piezo_shift_x_value - tracking_shift_x - Math.Round( piezo_shift_x_value - tracking_shift_x);
                    dlp.piezo_hist_y[2] =  piezo_shift_y_value - tracking_shift_y - Math.Round( piezo_shift_y_value - tracking_shift_y);
                    ...
                    //using (System.IO.StreamWriter file = new System.IO.StreamWriter(@"piezo_log.txt", true))
                    //{
                    //    //file.WriteLine("test");
                    //    file.WriteLine(piezo_shift_x.Value.ToString() + ", " + piezo_shift_y.Value.ToString());
                    //}

                    //Console.WriteLine("Avg piezo set point: (" + piezo_shift_x.Value.ToString() + ", " + piezo_shift_y.Value.ToString() + ") - " + File.GetLastWriteTime(log_file).ToString());
                    ...
                    try
                    {
                        ...
                        this.Invoke((MethodInvoker)delegate // BeginInvoke seems to give double events??
                        {
                            ...
                            //numericTrackingX.Value = (decimal)tracking_shift_x;
                            //numericTrackingY.Value = (decimal)tracking_shift_y;
                            //button_runTracked.PerformClick();

                            //string[] piezo_setpoints = System.IO.File.ReadAllText("piezo_log.txt").Trim().Split(',');


                            //List<double> piezo_hist_x = new List<double>();
                            //List<double> piezo_hist_y = new List<double>();

                            //piezo_hist_x.Add(Convert.ToDouble(tracking_shifts[0].Trim()));
                            //piezo_hist_y.Add(Convert.ToDouble(tracking_shifts[1].Trim()));
                            ...
                            //    for (k = 0; k < 3; k++)
                            //    {
                            //        piezo_avg_x = piezo_avg_x + piezo_hist_x[piezo_hist_x.Count() - k - 1];
                            //        piezo_avg_y = piezo_avg_y + piezo_hist_y[piezo_hist_y.Count() - k - 1];
                            //    }
                            //    k = 3;
                            //}
                            //else
                            //{
                            //    for (i = 0; k < dlp.tracking_shift_x.Count(); k++)
                            //    {
                            //        piezo_avg_x = piezo_avg_x + piezo_hist_x[piezo_hist_x.Count() - k - 1];
                            //        piezo_avg_y = piezo_avg_y + piezo_hist_y[piezo_hist_y.Count() - k - 1];
                            //    }
                            ...
                            //}
                            //piezo_avg_x = piezo_avg_x / (k);
                            //piezo_avg_y = piezo_avg_y / (k);
                            Console.WriteLine("Piezo x err hist (" + dlp.piezo_hist_x[0].ToString() + ", " + dlp.piezo_hist_x[1].ToString() + ", "  + dlp.piezo_hist_x[2].ToString() + ") - " + File.GetLastWriteTime(log_file).ToString());
                            Console.WriteLine("Piezo y err hist (" + dlp.piezo_hist_y[0].ToString() + ", " + dlp.piezo_hist_y[1].ToString() + ", " + dlp.piezo_hist_y[2].ToString() + ") - " + File.GetLastWriteTime(log_file).ToString());
                            Console.WriteLine("Avg piezo error x,y: (" + piezo_avg_x.ToString() + ", " + piezo_avg_y.ToString() + ") - " + File.GetLastWriteTime(log_file).ToString());
                            ...
                            //new error = old error + (set_point - actual) - round (set_point - actual)
                            //piezo_err_x.Value = piezo_err_x.Value + (decimal)((piezo_avg_x - tracking_shift_x) - Math.Round(piezo_avg_x - tracking_shift_x));
                            //piezo_err_y.Value = piezo_err_y.Value + (decimal)((piezo_avg_y - tracking_shift_y) - Math.Round(piezo_avg_y - tracking_shift_y));
                            ...
                            //piezo track
                            //piezo_shift_x.Value = piezo_shift_x.Value - (decimal)tracking_shift_x ;
                            //piezo_shift_y.Value = piezo_shift_y.Value - (decimal)tracking_shift_y ;
        ...
        private void button_runTracked_Click(object sender, EventArgs e)
        {
            ...
            System.IO.File.Copy("patterns\\tracking_hor.raw", "patterns\\current\\round_dmd_tracking.raw", true);
        ...
        public double piezo_shift_x_value = 0;
        public double piezo_shift_y_value = 0;

        private void Piezo_shift_Click(object sender, EventArgs e)
        {
            piezo_shift_x_value = (double) piezo_shift_x.Value;
            piezo_shift_y_value = (double) piezo_shift_y.Value;
            //dlp.tracking_shift_x.RemoveRange(0, dlp.tracking_shift_x.Count());
            //dlp.tracking_shift_y.RemoveRange(0, dlp.tracking_shift_y.Count());
            Piezo_shift_avg();
            //piezo shift code now moved to class below so clickling the shift button can erase the tracking counter
        }
        ...
        public void Piezo_shift_avg()
        {
            ...
            Vs = Piezo_convert(piezo_shift_x_value + (double)piezo_err_x.Value, piezo_shift_y_value + (double)piezo_err_y.Value);
            //Vs = Piezo_convert((double)piezo_shift_x.Value, (double)piezo_shift_y.Value);
            //shift so erase previous numbers
            //dlp.tracking_shift_x.RemoveRange(0, dlp.tracking_shift_x.Count()-1);
            //dlp.tracking_shift_y.RemoveRange(0, dlp.tracking_shift_y.Count()-1);
            ...
            if ((0 < Vs[0]) && (Vs[0] < 10) && (0 < Vs[1]) && (Vs[1] < 10))
            {
                ...
                dlp.Voltage(Vs[0], 2);
                dlp.Voltage(Vs[1], 3);
                Console.WriteLine("Channel 2 voltage (Vx): " + Vs[0].ToString());
                Console.WriteLine("Channel 3 voltage (Vy): " + Vs[1].ToString());
                Console.WriteLine(" ");
            }
            else
            {
        ...
        private double[] Piezo_convert(double x, double y)
        {
            // convert sites to piezo voltages
            double[] Vs = new double[2];
            // these values came from andor calibration
            //double Vx = -0 * Math.Pow(x, 3) + 0.027 * Math.Pow(x, 2) - 3.957232 * x;
            //double Vy = (-0.087651 * Math.Pow(y, 3) + 0.122744 * Math.Pow(y, 2) - 2.169453 * y);

            double Vx = -0.728 * x + 0.00348 * Math.Pow(x, 2);//new calibration values 8/17/15
            double Vy = -0.549 * y + 0.00184 * Math.Pow(y, 2);

            //double x_corr = 0.027573 * Vy;
            //double y_corr =0.001248 * Vx;

            //double Vx_corr = -0.223 * Math.Pow(x_corr, 3) + 0.268 * Math.Pow(x_corr, 2) - 2.601 * (x_corr);
            //double Vy_corr = -0.070886 * Math.Pow(y_corr, 3) + 0.154090 * Math.Pow(y_corr, 2) - 2.067410 * (y_corr);

            //double Vx_corr = -0.739701 * Math.Pow(x - x_corr, 3) + 0.478132 * Math.Pow(x - x_corr, 2) - 3.957232 * (x - x_corr);
            //double Vy_corr = -0.087651 * Math.Pow(y - y_corr, 3) + 0.122744 * Math.Pow(y - y_corr, 2) - 2.169453 * (y - y_corr);
        ...
        private void atomLineCalTrackButton_Click(object sender, EventArgs e)
        {
            ...
            {
                System.IO.File.Copy("patterns\\mapping\\ph,amp\\" + (j).ToString("D3") + ".raw", "patterns\\current\\" + j.ToString("D3") + "0" + "fringe" + ".raw", true);
                System.IO.File.Copy("patterns\\tracking_hor.raw", "patterns\\current\\" + j.ToString("D3") + "1" + "dmd_tracking" + ".raw", true);
            }

            System.IO.File.Copy("patterns\\grating\\000.raw", "patterns\\current\\00000gauss.raw", true);
            System.IO.File.Copy("patterns\\tracking_hor.raw", "patterns\\current\\00000tracking.raw", true);
        ...
        private void groupBox_latticeTracking_Enter(object sender, EventArgs e)
        {
