Public Function AddMottInsulatorSequenceHoldBlueDonut(ByVal t_start As Double,
    ...
    '2022/10/05 v3 correction. Do the same thing not only for QUIC axis but also QUAD Axis.

    '*********************************************************************************************************************************************************

    '************************************************************ Calculate time for 2D ramp ************************************************************

    Dim lattice1_midpoint_volt As Double = lattice1_voltage_offset + lattice1_calib_volt + 0.5 * Log10(twod_ramp_midpoint / lattice1_calib_depth)
    Dim lattice2_midpoint_volt As Double = lattice2_voltage_offset + lattice2_calib_volt + 0.5 * Log10(twod_ramp_midpoint / lattice2_calib_depth)

    Dim lattice1_endpoint_volt As Double = lattice1_voltage_offset + lattice1_calib_volt + 0.5 * Log10(twod_ramp_endpoint / lattice1_calib_depth)
    Dim lattice2_endpoint_volt As Double = lattice2_voltage_offset + lattice2_calib_volt + 0.5 * Log10(twod_ramp_endpoint / lattice2_calib_depth)

    Dim lattice1_max_volt As Double = lattice1_voltage_offset + lattice1_calib_volt + 0.5 * Log10(lattice1_max / lattice1_calib_depth)
    Dim lattice2_max_volt As Double = lattice2_voltage_offset + lattice2_calib_volt + 0.5 * Log10(lattice2_max / lattice2_calib_depth)

    Dim lattice1_start_volt As Double = lattice1_voltage_offset + lattice1_calib_volt + 0.5 * Log10(lattice1_start_depth / lattice1_calib_depth)
    Dim lattice2_start_volt As Double = lattice2_voltage_offset + lattice2_calib_volt + 0.5 * Log10(lattice2_start_depth / lattice2_calib_depth)
    ...
    Dim dipole_voltage_high As Double
    ...
    Dim twod_start_time As Double = dipole_ready_time + dimple_ramp_down + donut_ramp_down
    ...
    If MI_en = True Then
        ...
        analogdata.AddSmoothRamp(0.875, 0.03, t_start, t_start + cigar_move_time, ps3_ao)
        ...
        analogdata.AddSmoothRamp(ps6_scaler * quic_push * quic_mirror_ratio / 16.0 + ps6_offset,
    ps6_scaler * quic_axis_offset + ps6_offset, cigar_move_end_time + biglattice_ramp_on,
    cigar_move_end_time + biglattice_ramp_on + spherical_creation_time, ps6_ao)
        ...
        analogdata2.AddSmoothRamp(round_dimple_voltage, 1.7, dipole_ready_time, dipole_ready_time + dimple_ramp_down, round_dimple_power)

        '**********************************************Blue Doughnut******************************************
        dipole_voltage = red_calib_volt + Log10(red_freq / red_calib_freq)
        dipole_voltage_high = red_calib_volt + Log10(red_freq_high / red_calib_freq)
        ...
        analogdata.AddSmoothRamp(1.44, dipole_voltage_high, dipole_start_time, dipole_ready_time, red_dipole_power)
        analogdata.AddStep(dipole_voltage_high, dipole_ready_time, dipole_ready_time + dimple_ramp_down, red_dipole_power) 'calibration
        analogdata.AddSmoothRamp(dipole_voltage_high, dipole_voltage, dipole_ready_time + dimple_ramp_down, twod_start_time, red_dipole_power)

        ' Red dipole compensates during 2D lattice rampup 
        Dim dV As Double = dipole_volt_compensate * (1 - Log10(40 / lattice1_max) / Log10(40 / 0.01)) '40 Er is the measured compensation voltage for the BDT, only valid down to 0.4 Er lattice depth (which is the lattice 1 midpoint)
        ...
        analogdata.AddStep(ps6_scaler * quic_gravity_offset + ps6_offset, axial_start_time, twod_start_time - 20, ps6_ao)

        Dim quic_latt_grad_offset_start As Double = quic_latt_grad_offset * lattice2_start_depth / lattice2_max
        Dim quic_latt_grad_offset_mid As Double = quic_latt_grad_offset * twod_ramp_midpoint / lattice2_max

        analogdata.AddSmoothRamp(ps6_scaler * quic_gravity_offset + ps6_offset, ps6_scaler * (quic_gravity_offset + quic_latt_grad_offset_start) + ps6_offset, twod_start_time - 20, twod_start_time, ps6_ao)
        analogdata.AddRamp(ps6_scaler * (quic_gravity_offset + quic_latt_grad_offset_start) + ps6_offset, ps6_scaler * (quic_gravity_offset + quic_latt_grad_offset_mid) + ps6_offset, twod_start_time, twod_start_time + bandgap_open_rampup, ps6_ao)
        analogdata.AddSmoothRamp(ps6_scaler * (quic_gravity_offset + quic_latt_grad_offset_mid) + ps6_offset, ps6_scaler * (quic_gravity_offset + quic_latt_grad_offset) + ps6_offset, twod_start_time + bandgap_open_rampup, twodmax_ready_time, ps6_ao)
