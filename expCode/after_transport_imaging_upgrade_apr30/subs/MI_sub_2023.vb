Public Function MIsub2023(ByVal t_start As Double, _
...
'Version Notes
'2022/10/04 v2: Added a functionality to compensate lattice gradient in a more well-organized way than Julian's hack.
'2022/10/05 v3: Do the same thing not only for QUIC axis but also QUAD Axis.
'2023/01/13 v3p1: Cleaned up variables and defined 0s in a few channels.
...
analogdata.AddStep((0.8 * 2.342), t_start, dimple_ready_time, ps2_ao) 'Holds cigar trap on during evaporation
...
analogdata2.AddSmoothRamp(0, quic_push*ps5_scaler, t_start, cigar_move_end_time, ps5_ao)
analogdata2.AddStep(quic_push*ps5_scaler, cigar_move_end_time, dimple_ready_time, ps5_ao)
...
'Add a B-field gradient that cancels the lattice gradient (along quad axis)
...
'PS 2
analogdata.AddSmoothRamp((0.8 * 2.342), 0, dimple_ready_time, bfield_off_time, ps2_ao)'Ramp down PS2
...
analogdata2.AddStep(0, bfield_off_time, twodmax_ready_time, ps5_ao)
...
'Add a B-field gradient that cancels the lattice gradient (along quic axis)
