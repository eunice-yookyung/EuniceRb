Public Function AddMottInsulatorSequenceSFHoldEj1D(ByVal t_start As Double, _
    ByVal twod_ramp_holdpoint As Double, _
    ByVal twod_ramp_ejectpoint As Double, _
    ByVal hold_dur As Double, _
    ByVal hold_dur_ej As Double, _
...
Dim lattice1_holdpoint_volt As Double = lattice1_voltage_offset + lattice1_calib_volt+.5*Log10(twod_ramp_holdpoint/lattice1_calib_depth)
Dim lattice2_holdpoint_volt As Double = lattice2_voltage_offset + lattice2_calib_volt+.5*Log10(twod_ramp_holdpoint/lattice2_calib_depth)

Dim lattice1_ejectpoint_volt As Double = lattice1_voltage_offset + lattice1_calib_volt+.5*Log10(twod_ramp_ejectpoint/lattice1_calib_depth)
Dim lattice2_ejectpoint_volt As Double = lattice2_voltage_offset + lattice2_calib_volt+.5*Log10(twod_ramp_ejectpoint/lattice2_calib_depth)
...
Dim oned_start_time As Double = dipole_ready_time + dimple_ramp_down
Dim oned_log_end_time As Double = oned_start_time + bandgap_open_rampup
Dim onedmax_ready_time As Double = oned_log_end_time + twod_chopped_rampup

Dim twod_start_time As Double = onedmax_ready_time
'Dim twod_start_time As Double = dipole_ready_time + dimple_ramp_down
'keep ramp speed the same
Dim twod_log_end_time As Double = twod_start_time+bandgap_open_rampup
Dim rampV As Double = lattice1_max_volt-lattice1_midpoint_volt
Dim rampV1 As Double = lattice1_holdpoint_volt-lattice1_midpoint_volt

Dim rampV3 As Double = lattice1_holdpoint_volt- lattice1_ejectpoint_volt
Dim rampV4 As Double = lattice1_max_volt - lattice1_ejectpoint_volt

'from previous version
Dim rampV2 As Double = lattice1_max_volt - lattice1_holdpoint_volt


Dim twod_hold_start_time As Double = twod_log_end_time +  twod_chopped_rampup*((rampV1)/(rampV))
Dim twod_hold_end_time As Double = twod_hold_start_time + hold_dur
Dim twod_eject_down_start_time As Double = twod_hold_end_time
Dim twod_eject_down_end_time As Double = twod_eject_down_start_time + twod_chopped_rampup*((rampV3)/(rampV))
Dim twod_eject_up_start_time As Double = twod_eject_down_end_time +hold_dur_ej
Dim twodmax_ready_time As Double =twod_eject_up_start_time + twod_chopped_rampup*((rampV4)/(rampV))
...
Dim mid_dipole_voltage As Double = dipole_voltage + dV*(rampV1)/rampV
Dim ej_dipole_voltage As Double = dipole_voltage + dV*(lattice1_ejectpoint_volt-lattice1_midpoint_volt)/rampV


'analogdata.AddRamp(dipole_voltage, end_dipole_voltage, twod_start_time, twodmax_ready_time, red_dipole_power)
'chopped up ramp for hold
'analogdata.AddRamp(dipole_voltage, mid_dipole_voltage, twod_start_time, twod_hold_start_time, red_dipole_power)
'analogdata.AddStep(mid_dipole_voltage, twod_hold_start_time, twod_hold_end_time, red_dipole_power)
'analogdata.AddRamp(mid_dipole_voltage, end_dipole_voltage, twod_hold_end_time, twodmax_ready_time, red_dipole_power)


analogdata.AddRamp(dipole_voltage, mid_dipole_voltage, twod_start_time, twod_hold_start_time, red_dipole_power)
analogdata.AddStep(mid_dipole_voltage, twod_hold_start_time, twod_hold_end_time, red_dipole_power)
analogdata.AddRamp(mid_dipole_voltage, ej_dipole_voltage, twod_eject_down_start_time, twod_eject_down_end_time, red_dipole_power)
analogdata.AddStep(ej_dipole_voltage, twod_eject_down_end_time,twod_eject_up_start_time, red_dipole_power)
analogdata.AddRamp(ej_dipole_voltage, end_dipole_voltage, twod_eject_up_start_time, twodmax_ready_time, red_dipole_power)
'******************************************** ramp down bfields ***********************************************************************
'PS 1
analogdata.AddSmoothRamp(quad_axis_offset, quad_dimple_cleanup_volt, dimple_ready_time,bfield_off_time, ps1_ao)
...
'PS 7
analogdata.AddSmoothRamp(spherical_black_coil*ps7_scaler,0,dimple_ready_time,bfield_off_time, ps7_ao)
'Siphon
...
digitaldata2.AddPulse(lattice2D765_ttl2,oned_start_time-1,  twodmax_ready_time)
digitaldata2.AddPulse(lattice2D765_shutter,twod_start_time-20, twodmax_ready_time)
digitaldata2.AddPulse(lattice2D765_shutter2,oned_start_time-20, twodmax_ready_time)


analogdata.AddLogRamp(lattice2_start_volt,lattice2_midpoint_volt,oned_start_time,oned_log_end_time,lattice2D765_power2)
analogdata.AddRamp(lattice2_midpoint_volt,lattice2_max_volt,oned_log_end_time,twod_start_time,lattice2D765_power2)
analogdata.AddStep(lattice2_max_volt,twod_start_time,twodmax_ready_time,lattice2D765_power2)

analogdata.AddLogRamp(lattice1_start_volt,lattice1_midpoint_volt,twod_start_time,twod_log_end_time,lattice2D765_power)
'analogdata.AddLogRamp(lattice2_start_volt,lattice2_midpoint_volt,twod_start_time,twod_log_end_time,lattice2D765_power2)
'chopping up ramp for  hold time
analogdata.AddRamp(lattice1_midpoint_volt,lattice1_holdpoint_volt,twod_log_end_time,twod_hold_start_time,lattice2D765_power)
'analogdata.AddRamp(lattice2_midpoint_volt,lattice2_holdpoint_volt,twod_log_end_time,twod_hold_start_time,lattice2D765_power2)

analogdata.AddStep(lattice1_holdpoint_volt,twod_hold_start_time,twod_hold_end_time,lattice2D765_power)
'analogdata.AddStep(lattice2_holdpoint_volt,twod_hold_start_time,twod_hold_end_time,lattice2D765_power2)

'analogdata.AddRamp(lattice1_holdpoint_volt,lattice1_max_volt,twod_hold_end_time,twodmax_ready_time,lattice2D765_power)
'analogdata.AddRamp(lattice2_holdpoint_volt,lattice2_max_volt,twod_hold_end_time,twodmax_ready_time,lattice2D765_power2)
analogdata.AddRamp(lattice1_holdpoint_volt,lattice1_ejectpoint_volt,twod_eject_down_start_time,twod_eject_down_end_time,lattice2D765_power)
'analogdata.AddRamp(lattice2_holdpoint_volt,lattice2_ejectpoint_volt,twod_eject_down_start_time,twod_eject_down_end_time,lattice2D765_power2)

analogdata.AddStep(lattice1_ejectpoint_volt, twod_eject_down_end_time,twod_eject_up_start_time, lattice2D765_power)
'analogdata.AddStep(lattice2_ejectpoint_volt, twod_eject_down_end_time,twod_eject_up_start_time, lattice2D765_power2)

analogdata.AddRamp(lattice1_ejectpoint_volt,lattice1_max_volt,twod_eject_up_start_time,twodmax_ready_time,lattice2D765_power)
'analogdata.AddRamp(lattice2_ejectpoint_volt,lattice2_max_volt,twod_eject_up_start_time,twodmax_ready_time,lattice2D765_power2)
